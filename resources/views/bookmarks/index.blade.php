...
<script>
    ...
    fetch('/api/dashboard/bookmarks')
        ...
        .then(function(data) {
            ...
            // 处理 scope 权限不足错误
            if (data.error === 'scope_missing' || data._status === 403) {
                area.innerHTML = '<div class="text-center py-12">' +
                    ...
                    '</svg></div>' +
                    '<p class="text-xl text-yellow-300 mb-2">权限不足</p>' +
                    '<p class="text-blue-200/70">当前角色未授权书签权限</p>' +
                    '<p class="text-blue-200/70 text-sm mt-1">请在 EVE 授权页面重新授权并勾选书签相关权限</p>' +
                    '</div>';
                return;
            }
