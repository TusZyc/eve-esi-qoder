...
class WalletDataController extends Controller
{
     ...
     * 获取军团钱包余额（各部门）
     */
    public function corporationBalance(Request $request)
    {
        $user = $request->user();
        $characterId = $user->eve_character_id;
        $token = $user->access_token;
        ...
        try {
            ...
            if (!$characterInfo || empty($characterInfo['corporation_id'])) {
                return response()->json(['error' => '未加入军团'], 400);
            }
            ...
            if (!$hasAccess) {
                return response()->json([
                    'error' => '无军团钱包权限',
                    'message' => '需要 Director、Accountant 或 Junior Accountant 角色'
                ], 403);
            }

            // 获取军团名称
            $corpName = Cache::remember("corp_name_{$corporationId}", 86400, function () use ($baseUrl, $corporationId) {
                $response = Http::timeout(15)
                    ->get("{$baseUrl}corporations/{$corporationId}/", [
                        'datasource' => 'serenity'
                    ]);
                return $response->ok() ? ($response->json()['name'] ?? "军团 #{$corporationId}") : "军团 #{$corporationId}";
            });
            ...
            // 按部门排序
            usort($formattedWallets, fn($a, $b) => $a['division'] <=> $b['division']);
            ...
            return response()->json(['error' => '获取军团钱包余额失败: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function corporationJournal(Request $request)
    {
        ...
        try {
            ...
            if (!$hasAccess) {
                return response()->json(['error' => '无军团钱包权限'], 403);
            }
            ...
            return response()->json(['error' => '获取军团钱包流水失败: ' . $e->getMessage()], 500);
        }
    }
